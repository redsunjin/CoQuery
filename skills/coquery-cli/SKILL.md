...
- Treat PostgreSQL as experimental and limited to the documented `schema`, `schema_detail`, `query`, `insert`, `update`, `delete`, write-safety guard smoke paths plus `generate select_simple`, `generate count_simple`, and direct `generate join_inner` / `generate join_left` smoke slices.

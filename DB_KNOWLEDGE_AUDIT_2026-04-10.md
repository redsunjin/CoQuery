...
- a schema guard that validates generated and simple natural-language table/column identifiers against `schema_detail`
...
The local planner is wired into covered generation, natural, and write-planning paths, and `schema_detail` can expose normalized metadata. Generated SQL now validates basic identifiers, but templates are still shallow and not relationship-aware, join-aware, or expression-aware.
...
1. relationship-aware and join-aware generation using schema_detail foreign keys and constraints
...
- `schema detail aware generation seed exists`
